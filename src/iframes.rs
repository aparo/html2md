  ...
  fn handle(&mut self, tag: &Handle, printer: &mut StructuredPrinter) {
    ...
    if let Some(capture) = YOUTUBE_PATTERN.captures(&src) {
      ...
      printer.append_str(&format!(
        "[![Embedded YouTube video](https://img.youtube.com/vi/{mid}/0.jpg)](https://www.youtube.com/watch?v={mid})",
        mid = media_id
      ));
      return;
    }
    ...
    if let Some(capture) = VK_PATTERN.captures(&src) {
      ...
      printer.append_str(&format!(
        "[![Embedded VK video](https://st.vk.com/images/icons/video_empty_2x.png)](https://vk.com/video{oid}_{vid})",
        oid = owner_id,
        vid = video_id
      ));
      return;
